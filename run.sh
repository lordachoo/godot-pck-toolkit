...
while true; do
  ...
  case "$c" in
    ...
    10) need; "$PY" find_aes_rvas.py "$TARGET"; pause ;;
    11) need; read -rp "capture seconds [30]: " S; S="${S:-30}"
        "$PY" frida_keycatch.py "$TARGET" --seconds "$S"; pause ;;
    12) need; read -rp "key.bin [$OUT/key.bin]: " K; K="${K:-$OUT/key.bin}"
        "$PY" extract_godot_project.py "$TARGET" --key "$K" --out "$OUT/project"; pause ;;
    13) read -rp ".gdc file: " G; "$PY" gdc_inspect.py "$G" --strings; pause ;;
    14) need; read -rp "path to gdre_tools.exe: " GDRE; read -rp "key (64 hex, blank if none): " HK
    ...
    15) "$PY" -m pip install -r ../requirements.txt; pause ;;
    q|Q) exit 0 ;;
    *) ;;
  esac
done

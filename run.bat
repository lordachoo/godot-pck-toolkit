...
echo   10^) Locate AES-NI setkey entries ^(static, no spawn^)
echo   11^) Recover key by HOOKING a running game ^(Frida; beats key-wipe^)
echo   12^) Extract project    ^(needs key.bin^)
echo   13^) Inspect a .gdc file
echo   14^) Decompile via Godot RE Tools
echo   15^) Install Python dependencies
echo    q^) Quit
echo --------------------------------------------------------
set "c="
set /p "c=choice: "
...
if "%c%"=="10" ( call :need & %PY% find_aes_rvas.py "!TARGET!" & pause & goto menu )
if "%c%"=="11" ( call :need & set "S=30" & set /p "S=capture seconds [30]: " & %PY% frida_keycatch.py "!TARGET!" --seconds "!S!" & pause & goto menu )
if "%c%"=="12" ( call :need & set "K=%OUT%\key.bin" & set /p "K=key.bin [%OUT%\key.bin]: " & %PY% extract_godot_project.py "!TARGET!" --key "!K!" --out "%OUT%\project" & pause & goto menu )
if "%c%"=="13" ( set /p "G=.gdc file: " & %PY% gdc_inspect.py "!G!" --strings & pause & goto menu )
if "%c%"=="14" ( call :need & set /p "GDRE=path to gdre_tools.exe: " & set /p "HK=key (64 hex, blank if none): " & if defined HK ( %PY% decompile_gdc_batch.py --gdre "!GDRE!" --recover "!TARGET!" --key "!HK!" --out "%OUT%\recovered" ) else ( %PY% decompile_gdc_batch.py --gdre "!GDRE!" --recover "!TARGET!" --out "%OUT%\recovered" ) & pause & goto menu )
if "%c%"=="15" ( %PY% -m pip install -r ..\requirements.txt & pause & goto menu )
if /i "%c%"=="q" ( exit /b 0 )
goto menu
